...
function TodayInfo({ date, objects }) {
	...
	if (objects[0]) {
		...
		const sorted = Array.from(objects).sort(
			(x, y) => x.close_approach_data[0].epoch_date_close_approach - y.close_approach_data[0].epoch_date_close_approach
		);
	...
	if (nextApproachList[0] && pastApproaches[pastApproaches.length - 1]) {
		nextApproach = [
			{
				date: getFormattedDate(new Date(nextApproachList[0].close_approach_data[0].epoch_date_close_approach)),
				miss_distance: nextApproachList[0].close_approach_data[0].miss_distance.kilometers,
				relative_velocity: nextApproachList[0].close_approach_data[0].relative_velocity.kilometers_per_hour,
		...
		lastApproach = [
			{
				date: getFormattedDate(
					new Date(pastApproaches[pastApproaches.length - 1].close_approach_data[0].epoch_date_close_approach)
				),
				miss_distance: pastApproaches[pastApproaches.length - 1].close_approach_data[0].miss_distance.kilometers,
				relative_velocity:
					pastApproaches[pastApproaches.length - 1].close_approach_data[0].relative_velocity.kilometers_per_hour,
				...
				orbiting_body: pastApproaches[pastApproaches.length - 1].close_approach_data[0].orbiting_body,
			},
		];
	}
